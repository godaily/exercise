...
func (c *HomeAction) Login() error {
	if c.Method() == "GET" {
		...
		if err == nil {
			if has {
				session := c.Session()
				session.Set(USER_ID_TAG, c.User.Id)
				session.Set(USER_NAME_TAG, c.User.UserName)
				session.Set(USER_AVATAR_TAG, c.User.Avatar)
				return c.Go("root")
			}
...
func (c *HomeAction) Logout() error {
	session := c.Session()
	session.Del(USER_ID_TAG)
	session.Del(USER_NAME_TAG)
	session.Del(USER_AVATAR_TAG)
	return c.Go("root")
}
